...
// Vytvorenie nového záznamum textu
router.post('/', tokenExistsMiddleware, async (req, res) => {
    try {
        ...
        console.error('Chyba pri vytváraní nového textu:', err.message);
        res.status(500).json({ error: 'Interná chyba servera.' + err.message });
    }
});

// Ziskanie všetkych záznamov text z DB
router.get('/', tokenExistsMiddleware, async (req, res) => {
    try {
        ...
        res.status(500).send('Interná chyba servera.' + err.message);
    }
});
...
router.post('/insertMany', async (req, res) => {
    try {
        ...
        res.status(201).json("Saved {n} texts");
