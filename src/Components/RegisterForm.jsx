                            ...
                            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                                ...
                                <div>
                                    <label htmlFor="firstName"
                                           className="mb-2 text-fontSize16 text-custom-dark-blue flex justify-center">Krstné
                                        meno<span className="text-red-400 ml-1">*</span></label>
                                    <div className="relative">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <button type="button" id="btnNewPasswordRepeat"
                                ...
                                {error || success ? (
                                    <div className="flex justify-center mt-5 text-red-500">
                                        {error &&
                                            <p className="text-red-5+00 font-semibold text-center text-fontSize20">{error}</p>}
                                        {success &&
                                            <p className="text-green-500 font-semibold text-center text-fontSize20">{success}</p>}
                                    </div>

                                ) : (
                                    <div className="flex justify-center mt-5 text-red-500 invisible ">
                                        <p className="text-red-500 text-center text-fontSize20 font-semibold">Final
                                            state placeholder</p>
                                    </div>
                                )}
                            </form>
                        ) : (
                            <VerifyCodeForm setRegistrationDone={setRegistrationDone} formnData={formData}
                                            setShowVerification={setShowVerification}
                                            setPasswordMatch={setPasswordMatch} setSuccess={setSuccess}
                                            email={formData.email} firstName={formData.firstName}
                                            lastName={formData.lastName} password={formData.password}
                                            role={formData.role}/>
                        )}
                    </div>
                </motion.div>
