...
export function DemoLogin({setStep, setProgress, progress}) {
    ...
    const steps = [
        {
            target: 'input#email',
            content: 'Zadajte platný, zaregistrovaný e-mail.',
        },
        {
            target: 'input#password',
            content: 'Zadajte správne heslo.'
        },
        {
            target: 'button[type="submit"]',
            content: 'Kliknite na tlačidlo pre prihlásenie.',
        }
    ];
    ...
    useEffect(() => {
        ...
        const emailExists = existingUsers.some(user => user.email === validData.email);

        if (!emailExists) {
            localStorage.setItem('demoUsers', JSON.stringify([...existingUsers, validData]));
        }
    }, []);
    ...
    const validate = () => {
        ...
        const usersFromStorage = JSON.parse(localStorage.getItem("demoUsers") || "[]");
        const foundUser = usersFromStorage.find(user =>
            user.email === email && user.password === password
        );

        if (!foundUser && errs.length === 0) {
            errs.push("Zadané údaje nezodpovedajú žiadnemu zaregistrovanému používateľovi.");
        }
        ...
        if (errs.length === 0) {
            ...
            setProgress(prev => {
                const updated = {
                    ...prev,
                    login: 'completed',
                    textUpload: prev.textUpload === 'locked' ? 'unlocked' : prev.textUpload
                };
    ...
    return (
        ...
        <div className="bg-white rounded-xl shadow-lg px-10 pb-10 space-y-6 max-w-screen-sm mx-auto">
            ...
            <p className="text-custom-dark-blue text-center font-light">
                Vyskúšajte si, ako funguje prihlásenie používateľa. Môžete použiť demo údaje alebo zadať vlastné.
            </p>
            <div className="bg-gray-200 rounded-3xl p-4 shadow-sm border border-custom-dark-blue-hover">
                <h3 className="text-lg font-bold text-custom-dark-blue mb-3">Požiadavky na prihlásenie:</h3>
                <ul className="space-y-2 text-custom-dark-blue">
                    <li className="flex items-start gap-2">
                        ...
                        <span>Zadaná emailová adresa musí byť<span className="font-bold"> validná</span>.</span>
                    </li>
                    <li className="flex items-start gap-2">
                        <FaCheckCircle className="text-custom-dark-blue w-3 h-3 flex-shrink-0 mt-1.5"/>
                        <span>Musí existovať účeť <span className="font-bold"> zaregistrovaný</span> pod zadanou emailovou adresou (testovací účet vytvorený <button
                            onClick={() => {
                                setStep('register')
                            }} className="underline">tu</button>, nie reálny účet).</span>
                    </li>
                    <li className="flex items-start gap-2">
                        <FaCheckCircle className="text-custom-dark-blue w-3 h-3 flex-shrink-0 mt-1.5"/>
                        <span>Zadané údaje musia byť správne.</span>
                    </li>
                </ul>
            </div>
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input type="text" name="email" id="email" autoComplete={"email"} value={email}
                               ...
                               placeholder="meno@domena.com" required="" onChange={(e) => setEmail(e.target.value)}/>
                    </div>
                </div>
                <div>
            ...
            <div className="min-h-[5rem] transition-all">
                ...
                {success && (
                    <div className="bg-green-100 border border-green-300 text-green-900 p-4 rounded-xl space-y-2 mt-4">
                        <p className="font-semibold">
                            Boli ste úspešne prihlásený ako: <span className="underline">{email}</span>
                        </p>
                        <p>
                            <strong>Váš token:</strong> <code
                            className="bg-white px-2 py-1 rounded-xl border">{token}</code>
                        </p>
